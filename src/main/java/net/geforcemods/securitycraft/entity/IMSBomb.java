 ...
 * The upstream keeps the owner in a custom synched data serializer; that registry does not exist on Fabric 1.20.1,
 * so the owner is a plain server-side field here. Its lifetime is the same as upstream's (not written to NBT).
 */
public class IMSBomb extends Fireball {
	...
	public IMSBomb(Level level, double x, double y, double z, double accelerationX, double accelerationY, double accelerationZ, int height, IMSBlockEntity be) {
		super(SCContent.IMS_BOMB_ENTITY, x, y, z, new Vec3(accelerationX, accelerationY, accelerationZ), level);
