...
/**
 * 1:1 with the upstream {@code entity.IMSBomb}: launches straight up, then homes onto its target. Upstream keeps the
 * owner in a custom synched data serializer; that serializer registry does not exist on Fabric, so the owner is a
 * plain server-side field here. Its lifetime is the same as upstream's (not written to NBT).
 */
