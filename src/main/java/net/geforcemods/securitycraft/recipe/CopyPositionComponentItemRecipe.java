...
public class CopyPositionComponentItemRecipe extends CombineRecipe {
	...
	private final Predicate<ItemStack> isDataEmpty;

	public CopyPositionComponentItemRecipe(CraftingBookCategory craftingBookCategory, Item item, Predicate<ItemStack> hasData) {
	...
	public static CopyPositionComponentItemRecipe mineRemoteAccessTool(CraftingBookCategory craftingBookCategory) {
		return new CopyPositionComponentItemRecipe(craftingBookCategory, SCContent.MINE_REMOTE_ACCESS_TOOL, stack -> !stack.getOrDefault(SCContent.BOUND_MINES, BoundMines.EMPTY).positions().isEmpty());
	}

	@Override
	public boolean matchesFirstItem(ItemStack stack) {
		return stack.is(item) && !isDataEmpty.test(stack);
	}
