...
/**
 * 1:1 with the upstream {@code blockentities.ClaymoreBlockEntity}. Fabric changes: the NeoForge item-handler
 * capability (which only served hoppers inserting lenses) is dropped, the menu is opened through Fabric's
 * {@link ExtendedMenuProvider} instead of {@code MenuProvider}, and lens save/load follow the per-slot
 * {@code ValueOutput}/{@code ValueInput} pattern already used by {@code LaserBlockBlockEntity}'s lens container.
 */
public class ClaymoreBlockEntity extends CustomizableBlockEntity implements ITickingBlockEntity, ExtendedMenuProvider<BlockPos>, SingleLensContainer {
	...
	public ClaymoreBlockEntity(BlockPos pos, BlockState state) {
		super(SCContent.CLAYMORE_BLOCK_ENTITY, pos, state);
		lens.setChangedListener(this::containerChanged);
	}
	...
	public void saveAdditional(ValueOutput output) {
		...
		ItemStack lensStack = lens.getItem(0);

		if (!lensStack.isEmpty())
			output.store("lens", ItemStack.CODEC, lensStack);
	}

	@Override
	public void loadAdditional(ValueInput input) {
		super.loadAdditional(input);
		cooldown = input.getIntOr("cooldown", -1);
		lens.setItemExclusively(0, input.read("lens", ItemStack.CODEC).orElse(ItemStack.EMPTY));
		lens.setChanged();
	}
	...
	public void preRemoveSideEffects(BlockPos pos, BlockState state) {
		if (level != null)
			Containers.dropContents(level, pos, lens);
