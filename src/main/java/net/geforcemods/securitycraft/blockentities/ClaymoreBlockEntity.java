...
/**
 * 1:1 with the upstream {@code blockentities.ClaymoreBlockEntity}. Fabric changes: the Forge item-handler capabilities
 * (which only served hoppers inserting lenses) are dropped, the menu is opened through Fabric's
 * {@link ExtendedScreenHandlerFactory} instead of {@code NetworkHooks}, and {@code getRenderBoundingBox} is dropped
 * because {@code ClaymoreRenderer#shouldRenderOffScreen} already opts the renderer out of frustum culling.
 */
public class ClaymoreBlockEntity extends CustomizableBlockEntity implements ITickingBlockEntity, ExtendedScreenHandlerFactory<BlockPos>, ContainerListener, SingleLensContainer {
	...
	public void saveAdditional(CompoundTag tag, HolderLookup.Provider lookupProvider) {
		...
		if (!lensStack.isEmpty())
			tag.put("lens", lensStack.save(lookupProvider));
	}
	...
	public void loadAdditional(CompoundTag tag, HolderLookup.Provider lookupProvider) {
		...
		lens.setItemExclusively(0, net.geforcemods.securitycraft.util.Utils.parseOptional(lookupProvider, tag.getCompoundOrEmpty("lens")));
		lens.setChanged();
	}
