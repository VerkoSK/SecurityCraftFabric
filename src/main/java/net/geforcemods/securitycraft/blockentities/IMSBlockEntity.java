...
/**
 * 1:1 with the upstream {@code blockentities.IMSBlockEntity}. The only Fabric change is the auto-restock branch, which
 * pulls a Bouncing Betty out of the container below through the Fabric Transfer API instead of Forge's
 * {@code ForgeCapabilities.ITEM_HANDLER}.
 */
public class IMSBlockEntity extends CustomizableBlockEntity implements ITickingBlockEntity {
	...
	public void loadAdditional(CompoundTag tag, HolderLookup.Provider lookupProvider) {
		...
		tag.getInt("targetingOption").ifPresent(ordinal -> targetingMode.setValue(TargetingMode.values()[ordinal]));
	}
