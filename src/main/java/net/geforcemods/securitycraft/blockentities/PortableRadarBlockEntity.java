...
/**
 * 1:1 with the upstream {@code blockentities.PortableRadarBlockEntity}; the only change is reading the flat
 * {@code ConfigHandler.enableTeamOwnership} field instead of Forge's {@code ConfigHandler.SERVER.enableTeamOwnership}.
 */
public class PortableRadarBlockEntity extends CustomizableBlockEntity implements ITickingBlockEntity {
	...
	private BooleanOption sendToTeamMembersOption = new BooleanOption("sendToTeamMembers", true) {
		@Override
		public Boolean get() {
			return ConfigHandler.enableTeamOwnership && super.get();
		}
	};
	...
	public PortableRadarBlockEntity(BlockPos pos, BlockState state) {
		super(SCContent.PORTABLE_RADAR_BLOCK_ENTITY, pos, state);
	}
				...
				for (Player closebyPlayer : closebyPlayers) {
					if (shouldSendMessage(closebyPlayer)) {
						...
						if (!messageReceivers.isEmpty())
							messageReceivers.forEach(player -> PlayerUtils.sendMessageToPlayer(player, Utils.localize(SCContent.PORTABLE_RADAR.getDescriptionId()), text, ChatFormatting.BLUE));
					}
				}
	...
	public void readOptions(CompoundTag tag) {
		tag.getBoolean("enabled").ifPresent(enabled -> tag.putBoolean("disabled", !enabled)); //legacy support
