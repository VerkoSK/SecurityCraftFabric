...
public class CallbackCheckbox extends AbstractButton {
	...
	protected void extractContents(GuiGraphicsExtractor extractor, int mouseX, int mouseY, float partialTicks) {
		...
		extractor.blitSprite(RenderPipelines.GUI_TEXTURED, sprite, getX(), getY(), width, height);
		extractor.text(minecraft.font, getMessage(), getX() + (int) (width * 1.2F), getY() + (height - 8) / 2, 0xFF000000 | textColor, false);
	}
