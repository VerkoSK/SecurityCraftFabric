...
public class ColorChooser extends Screen {
	...
	protected void init() {
		...
		rgbHexBox.setResponder(string -> {
			// setFilter is gone in 26.2, so validate here to avoid parsing non-hex input.
			if (updating || string.isEmpty() || !string.matches("[0-9a-fA-F]*"))
				return;
	...
	private Consumer<String> makeRgbResponder(EditBox box) {
		return string -> {
			// setFilter is gone in 26.2, so validate here to avoid parsing non-numeric input.
			if (updating || string.isEmpty() || !isByteString(string))
				return;
	...
	public void extractBackground(GuiGraphicsExtractor extractor, int mouseX, int mouseY, float partialTick) {
		extractor.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, xStart, yStart, 0.0F, 0.0F, 145, 109, 256, 256);
		ClientUtils.fillHorizontalGradient(extractor, 0, colorFieldLeft, colorFieldTop, colorFieldRight + 1, colorFieldBottom + 1, 0xFFFFFFFF, ClientUtils.HSBtoRGB(h, 1.0F, 1.0F) | 0xFF000000);
		extractor.fillGradient(colorFieldLeft, colorFieldTop, colorFieldRight + 1, colorFieldBottom + 1, 0x00000000, 0xFF000000);
	}
	...
	class HueSlider extends AbstractSliderButton {
		...
		public void extractWidgetRenderState(GuiGraphicsExtractor extractor, int mouseX, int mouseY, float partialTick) {
			extractor.blitSprite(RenderPipelines.GUI_TEXTURED, isHoveredOrFocused() ? HUE_SLIDER_HIGHLIGHTED_SPRITE : HUE_SLIDER_SPRITE, getX() + (int) (value * (width - 8)), getY(), 6, height);
		}
	}
}
