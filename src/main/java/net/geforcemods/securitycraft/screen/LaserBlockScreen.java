...
public class LaserBlockScreen extends AbstractContainerScreen<LaserBlockMenu> {
	private static final Identifier TEXTURE = SCContent.id("textures/gui/container/laser_block.png");
	private static final Identifier LENS_SLOT = SCContent.id("slot/lens");
	...
	public void extractRenderState(GuiGraphicsExtractor extractor, int mouseX, int mouseY, float partialTick) {
		...
		for (int i = 0; i < 6; i++) {
			if (be.getLensContainer().getItem(i).isEmpty())
				extractor.blitSprite(RenderPipelines.GUI_TEXTURED, LENS_SLOT, leftPos + 15, topPos + i * 22 + 27, 16, 16);
		}

		super.extractRenderState(extractor, mouseX, mouseY, partialTick);
		ClientUtils.renderModuleInfo(extractor, font, ModuleType.SMART, smartModuleTooltip, hasSmartModule, leftPos + 5, topPos + 5, mouseX, mouseY);
	}
