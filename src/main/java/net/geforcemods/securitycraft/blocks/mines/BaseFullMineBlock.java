 ...
 * Forge-only {@code getAppearance} and the Jade/WAILA {@code IOverlayDisplay} implementation (neither has a Fabric
 * counterpart in this port), and with {@code onDestroyedByPlayer}/the 5-arg {@code getCloneItemStack} expressed
 * through their vanilla equivalents.
 */
public class BaseFullMineBlock extends ExplosiveBlock implements IBlockMine {
	 ...
	 * are neither in creative mode nor the owner. That check needs the player, which the Forge-only 5-arg overload
	 * supplies and the vanilla hook does not, so the disguise is always handed out here.
	 */
	@Override
