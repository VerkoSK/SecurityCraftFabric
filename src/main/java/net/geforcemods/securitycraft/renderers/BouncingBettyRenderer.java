...
/** 1:1 with the original SecurityCraft {@code renderers.BouncingBettyRenderer}: draws the mine block with the TNT flash. */
public class BouncingBettyRenderer extends EntityRenderer<BouncingBetty> {
	...
	public void render(BouncingBetty entity, float entityYaw, float partialTicks, PoseStack pose, MultiBufferSource buffer, int packedLight) {
		...
		pose.mulPose(Axis.YP.rotationDegrees(90.0F));
		TntMinecartRenderer.renderWhiteSolidBlock(blockRenderer, SCContent.BOUNCING_BETTY.defaultBlockState(), pose, buffer, packedLight, fuse / 5 % 2 == 0);
		pose.popPose();
