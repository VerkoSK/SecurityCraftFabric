...
/** 1:1 with the upstream {@code renderers.BouncingBettyRenderer}. */
public class BouncingBettyRenderer extends EntityRenderer<BouncingBetty, BouncingBettyRenderState> {
	...
	public void render(BouncingBettyRenderState state, PoseStack pose, MultiBufferSource buffer, int packedLight) {
		...
		pose.mulPose(Axis.YP.rotationDegrees(90.0F));
		TntMinecartRenderer.renderWhiteSolidBlock(blockRenderer, SCContent.BOUNCING_BETTY.defaultBlockState(), pose, buffer, packedLight, (int) state.fuseRemainingInTicks / 5 % 2 == 0);
		pose.popPose();
