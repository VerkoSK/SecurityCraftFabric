...
/**
 * 1:1 with the original SecurityCraft {@code renderers.ClaymoreRenderer}: the two tripwire lines in front of an armed
 * claymore. The lens colour is read through {@link Utils} because this port stores it in NBT instead of using
 * {@code DyeableLeatherItem}.
 */
public class ClaymoreRenderer implements BlockEntityRenderer<ClaymoreBlockEntity> {
	...
	public void render(ClaymoreBlockEntity be, float partialTicks, PoseStack pose, MultiBufferSource buffer, int combinedLight, int combinedOverlay) {
		...
		if (Utils.hasLensColor(lens)) {
			int color = Utils.getLensColor(lens);
