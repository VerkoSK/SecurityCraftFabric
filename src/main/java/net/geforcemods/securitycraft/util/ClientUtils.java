...
public class ClientUtils {
	...
	public static void renderModuleInfo(GuiGraphicsExtractor extractor, Font font, ModuleType module, Component tooltip, boolean hasModule, int x, int y, int mouseX, int mouseY) {
		float alpha = hasModule ? 1.0F : 0.5F;
		int color = ARGB.colorFromFloat(alpha, 1.0F, 1.0F, 1.0F);

		// The guiTextured render type already blends, so the per-blit color alpha fades the icon (matches upstream renderModuleInfo).
		extractor.blit(RenderPipelines.GUI_TEXTURED, moduleTexture(module), x, y, 0.0F, 0.0F, 16, 16, 16, 16, color);

		if (module == ModuleType.REDSTONE)
			extractor.blit(RenderPipelines.GUI_TEXTURED, net.minecraft.resources.Identifier.withDefaultNamespace("textures/item/redstone.png"), x, y, 0.0F, 0.0F, 16, 16, 16, 16, color);
		else if (module == ModuleType.SPEED)
			extractor.blit(RenderPipelines.GUI_TEXTURED, net.minecraft.resources.Identifier.withDefaultNamespace("textures/item/sugar.png"), x, y, 0.0F, 0.0F, 16, 16, 16, 16, color);

		if (tooltip != null && mouseX >= x && mouseX < x + 16 && mouseY >= y && mouseY <= y + 16)
			extractor.setComponentTooltipForNextFrame(font, java.util.List.of(tooltip), mouseX, mouseY);
	}

	private static net.minecraft.resources.Identifier moduleTexture(ModuleType module) {
	...
	public static void fillHorizontalGradient(GuiGraphicsExtractor extractor, int unused, int left, int top, int right, int bottom, int leftColor, int rightColor) {
		...
		for (int x = 0; x < width; x++) {
			...
			extractor.fill(left + x, top, left + x + 1, bottom, color);
		}
	}
