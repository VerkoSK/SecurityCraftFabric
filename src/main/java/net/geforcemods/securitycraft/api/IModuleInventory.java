...
public interface IModuleInventory {
	...
	public default NonNullList<ItemStack> readModuleInventory(ValueInput input) {
		NonNullList<ItemStack> modules = NonNullList.withSize(getMaxNumberOfModules(), ItemStack.EMPTY);

		for (ValueInput entry : input.childrenListOrEmpty("Modules")) {
			byte slot = entry.getByteOr("ModuleSlot", (byte) 0);

			if (slot >= 0 && slot < modules.size())
				modules.set(slot, entry.read(ItemStack.MAP_CODEC).orElse(ItemStack.EMPTY));
		}

		return modules;
	}

	public default Map<ModuleType, Boolean> readModuleStates(ValueInput input) {
		...
		for (ModuleType module : ModuleType.values()) {
			if (acceptedModules.contains(module)) {
				...
				//if the module is accepted but no state was saved yet, revert to whether the module is installed
				moduleStates.put(module, input.getBooleanOr(key, hasModule(module)));
			}
			else
	...
	public default void writeModuleInventory(ValueOutput output) {
		ValueOutput.ValueOutputList list = output.childrenList("Modules");
		NonNullList<ItemStack> modules = getInventory();
		...
		for (int i = 0; i < modules.size(); i++) {
			if (!modules.get(i).isEmpty()) {
				...
				entry.putByte("ModuleSlot", (byte) i);
				entry.store(ItemStack.MAP_CODEC, modules.get(i));
			}
		}
	}

	public default void writeModuleStates(ValueOutput output) {
		for (ModuleType module : acceptedModules()) {
			output.putBoolean(module.name().toLowerCase() + "Enabled", isModuleEnabled(module));
		}
	}
