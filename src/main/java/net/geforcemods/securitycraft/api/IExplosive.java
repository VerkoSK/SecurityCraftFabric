...
/**
 * Enables a Block to be remotely detonated using SecurityCraft's mine remote access tool. Adapted from upstream's
 * {@code api.IExplosive} for Fabric, where {@code SCContent} fields are plain statics instead of deferred registry
 * objects, so the {@code .get()} calls upstream uses are dropped.
 *
 * @author Geforce
 */
