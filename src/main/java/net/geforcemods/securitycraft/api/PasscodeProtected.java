...
public interface PasscodeProtected {
	...
	default void onIncorrectPasscodeEntered(Player player) {
		if (this instanceof IModuleInventory moduleInv) {
			...
			if (moduleInv.isModuleEnabled(ModuleType.HARMING) && player instanceof net.minecraft.server.level.ServerPlayer serverPlayer && serverPlayer.hurtServer(serverPlayer.level(), CustomDamageSources.incorrectPasscode(serverPlayer.level().registryAccess()), ConfigHandler.incorrectPasscodeDamage))
