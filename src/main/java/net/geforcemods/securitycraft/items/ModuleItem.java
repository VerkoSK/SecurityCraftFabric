...
public class ModuleItem extends Item {
	...
	public InteractionResult use(net.minecraft.world.level.Level level, net.minecraft.world.entity.player.Player player, net.minecraft.world.InteractionHand hand) {
		...
		if (canBeCustomized() && (module == ModuleType.ALLOWLIST || module == ModuleType.DENYLIST)) {
			...
			if (!level.isClientSide())
				player.openMenu(new net.minecraft.world.SimpleMenuProvider((windowId, inv, p) -> new net.geforcemods.securitycraft.inventory.DisguiseModuleMenu(windowId, inv, new net.geforcemods.securitycraft.inventory.ModuleItemContainer(stack)), Component.translatable(getDescriptionId())));
	...
	public void appendHoverText(ItemStack stack, TooltipContext ctx, TooltipDisplay display, Consumer<Component> list, TooltipFlag flag) {
		if (containsCustomData || canBeCustomized())
			list.accept(MODIFIABLE);
		else
			list.accept(NOT_MODIFIABLE);

		if (canBeCustomized()) {
			Block addon = getBlockAddon(stack);

			if (addon != null)
				list.accept(Utils.localize("tooltip.securitycraft:module.itemAddons.added", Utils.localize(addon.getDescriptionId())).setStyle(Utils.GRAY_STYLE));
		}
		...
		if (containsCustomData) {
			...
			if (listModuleData != null)
				listModuleData.addToTooltip(ctx, list, flag, stack.getComponents());
		}
	}
